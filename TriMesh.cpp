...
void TriMesh::generateDisk(int num_division, float radius)
{
	...
	vertex_colors.push_back(glm::vec3(1, 1, 1));
	vertex_textures.push_back(glm::vec2(0.5, 0.5)); // 纹理中心

	// 圆周顶点 Index 1 ~ num_samples
	for (int i = 0; i < num_samples; i++) {
		float theta = i * step;
		float x = radius * cos(theta);
		float y = radius * sin(theta);
		vertex_positions.push_back(glm::vec3(x, y, 0));
		vertex_normals.push_back(glm::vec3(0, 0, 1));
		vertex_colors.push_back(glm::vec3(1, 1, 1));
	...
	// 2. 生成面片索引
	for (int i = 0; i < num_samples; i++) {
		// 构成三角形：中心点(0) -> 当前圆周点(i+1) -> 下一个圆周点
		int current = i + 1;
		int next = (i + 1) % num_samples + 1; // 最后一个点连回第一个点(Index 1)
		faces.push_back(vec3i(0, current, next));
	}
...
void TriMesh::generateCone(int num_division, float radius, float height)
{
	...
	// 1. 生成位置顶点 (Tip + Base Vertices)
	// Tip Index 0 (放在 Height 处)
	vertex_positions.push_back(glm::vec3(0, 0, height));
	vertex_normals.push_back(glm::vec3(0, 0, 1));
	vertex_colors.push_back(glm::vec3(1, 1, 1));

	// Base Vertices Index 1 ~ num_samples (放在 Z=0 处)
	for (int i = 0; i < num_samples; i++) {
		float theta = i * step;
		float x = radius * cos(theta);
		float y = radius * sin(theta);
		...
		// 计算侧面法向量 (近似)
		// 切线方向 (-sin, cos, 0), 母线方向 (-x, -y, height)
		// 法线 = (height*cos, height*sin, radius)
		glm::vec3 n = glm::normalize(glm::vec3(height * cos(theta), height * sin(theta), radius));
		vertex_normals.push_back(n);
		vertex_colors.push_back(glm::vec3(1, 1, 1));
	}

	// 2. 生成面片和对应的独立纹理坐标
	// 圆锥展开后，顶点映射到纹理上方中心，底面映射到下方
	for (int i = 0; i < num_samples; i++) {
		int current = i + 1;
		...
		// 纹理坐标 (由于展开处有接缝，不能共用顶点纹理，需单独推入)
		// Tip UV: (0.5, 1.0)
		vertex_textures.push_back(glm::vec2(0.5, 1.0));
		// Current Base UV: (i/N, 0.0)
		vertex_textures.push_back(glm::vec2((float)i / num_samples, 0.0));
		// Next Base UV: ((i+1)/N, 0.0)
		vertex_textures.push_back(glm::vec2((float)(i + 1) / num_samples, 0.0));

		// 记录这三个新纹理坐标的索引
		int start_tex_idx = vertex_textures.size() - 3;
		texture_index.push_back(vec3i(start_tex_idx, start_tex_idx + 1, start_tex_idx + 2));
	}
