...
void init()
{
	...
	// @TODO: Task2 生成圆盘并贴图
	disk->generateDisk(50, 0.5);
	// 按照图示将圆盘放在中间偏右位置，与圆柱并排
	disk->setTranslation(glm::vec3(0.6, 0.0, 0.0));
	// 圆盘生成在XY平面，默认朝向Z轴，符合要求不需要旋转
	...
	painter->addMesh(disk, "mesh_disk", "./assets/disk.jpg", vshader, fshader);
	meshList.push_back(disk);

	TriMesh* cone = new TriMesh();
	// @TODO: Task2 生成圆锥并贴图
	cone->generateCone(50, 0.5, 1.0);
	// 按照图示将圆锥放在最右侧
	cone->setTranslation(glm::vec3(1.8, -0.5, 0.0));
	// 圆锥生成是Z轴朝上，需要绕X轴旋转90度使其“站立”面向屏幕，类似于圆柱的处理
	...
	painter->addMesh(cone, "mesh_cone", "./assets/cone.jpg", vshader, fshader);
...
int main(int argc, char** argv)
{
	...
	// 配置窗口属性
	GLFWwindow* window = glfwCreateWindow(600, 600, "2023040114_王荟斐_实验4.1", NULL, NULL);
	if (window == NULL)
	{
